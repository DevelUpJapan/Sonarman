...
<?php
        $command = 'ps -ef| grep "tshark -i" | grep -v grep | wc -l';
        $before_vpn_status = array();
        $ret = null;
        exec($command, $before_vpn_status, $ret);
        if ($before_vpn_status[0] !== '0' ) {
            $status='Alive';
        }
        ...
        <div class="col-xs-8">
            <div class="panel panel-info">
                <div class="panel-heading">
                    <div class="panel-title">Capture Download<br>
                        Capture Status &nbsp; $status
                        <form action="/sonarman/post/changecap.php" method="post">
                        <input type=hidden name="changecap" value="changecap">
...
function paging($limit, $page, $disp=5){
    ...
    //Link for last page
    if($limit > $end){
        if($limit-1 > $end ) print "...";    //Display dot
...
function disp_log($page,$page,$max){
    ...
    for($i=$start;$i<$end;$i++){
        ...
        //print $logdata[$i][0]."<br>";
          print('<li><a href=/sonarman/cap/'.$logdata[$i][0].'>'.$logdata[$i][0]."</a><font size='-1'>");
          echo'&nbsp'.$logdata[$i][1].'&nbsp;&nbsp;';  
          echo round(filesize('/var/www/sonarman/cap/'.$logdata[$i][0]) / 1024) . "KB &nbsp;";
          print('</font> </li>');
